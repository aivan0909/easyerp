...
ALTER TABLE xmda_t ADD COLUMN IF NOT EXISTS xmdavoidid  varchar(20);
ALTER TABLE xmda_t ADD COLUMN IF NOT EXISTS xmdavoiddt  timestamptz;

-- ?°Ë≥º?ÆÈ†≠ÔºöË?‰ΩúÂª¢??‰ΩúÂª¢?ÇÈ?(Â∞çÁ®±)
ALTER TABLE pmdl_t ADD COLUMN IF NOT EXISTS pmdlvoidid  varchar(20);
...
BEGIN
  ...
  LOOP
    -- ?çÂ?Â∫´Â??∞Â?(???)
    PERFORM fn_stock_out(p_ent, v_line.pmdt001, v_header.pmds004, v_line.pmdt003, 'GR_VOID', p_docno, v_line.pmdtseq, '?ñÊ??∂Ë≤®?ÑÂ?');
...
BEGIN
  ...
  LOOP
    -- ?çÂ?Â∫´Â??∞Â?(?†Â?)
    PERFORM fn_stock_in(p_ent, v_line.xmdl001, v_header.xmdk006, v_line.xmdl002, 'SH_VOID', p_docno, v_line.xmdlseq, '?ñÊ??∫Ë≤®?ÑÂ?');
...
-- ============================================================
-- migration_cost.sql ???ΩÂú∞?êÊú¨ + ÁßªÂ??†Ê?Âπ≥Â??êÊú¨ ?Ä?ÄÊ¨Ñ‰?
-- ?®ÁΩ≤?ÜÂ?Ôº?.. -> void_functions.sql -> migration_cost.sql -> cost_functions.sql
-- ============================================================

-- ?∂Ë≤®?ÆÈ†≠ÔºöË??åÈ?Â§ñË≤ª?®Á∏ΩÈ°ç„Ä??ãË≤ª/?úÁ?Á≠?‰∏ÄÊ¨°ÊÄßË≤ª??‰æùÈ?È°çÊ?‰æãÂ??§Âà∞?ÑË?)
ALTER TABLE pmds_t ADD COLUMN IF NOT EXISTS pmds006 numeric(15,2) DEFAULT 0;

-- Â∫´Â?ÂΩôÁ∏ΩË°®Ô?Ë£ú„ÄåÁßª?ïÂ?Ê¨äÂπ≥?áÊ??¨„ÄçÔ?ÊØèÊ¨°?∂Ë≤®ÂæåÈ??∞Ë?ÁÆ?ALTER TABLE inaj_t ADD COLUMN IF NOT EXISTS inaj006 numeric(15,4) DEFAULT 0;

-- ?∫Ë≤®?ÆË°®Ë∫´Ô?Ë£ú„ÄåÊú¨Ê¨°Âá∫Ë≤®Ê??¨È?È°ç„Ä?COGS)ÔºåÂá∫Ë≤®Á¢∫Ë™çÁï∂‰∏ã‰?Âπ≥Â??êÊú¨Ë®àÁ?‰∏¶ÂØ´?•Ô?
-- ‰πãÂ?ÊØõÂà©?±Ë°®?¥Êé•ËÆÄ?ôÂÄãÊ?‰ΩçÔ?‰∏çÁî®?çÁ?
ALTER TABLE xmdl_t ADD COLUMN IF NOT EXISTS xmdl006 numeric(15,2);

-- ?∂Ë≤®?ÆË°®Ë∫´Ô?Ë£ú„ÄåÊú¨Ë°åÂ??§Âà∞?ÑËêΩ?∞Ê??¨„ÄçË??åÊú¨Ë°åËêΩ?∞ÂñÆ‰ΩçÊ??¨„ÄçÔ??π‰æø?•Â?ËøΩÊ∫Ø/Á®ΩÊ†∏
ALTER TABLE pmdt_t ADD COLUMN IF NOT EXISTS pmdt007 numeric(15,2) DEFAULT 0; -- ?ÜÊî§?∞Á?È°çÂ?Ë≤ªÁî®
ALTER TABLE pmdt_t ADD COLUMN IF NOT EXISTS pmdt008 numeric(15,4);          -- ?ΩÂú∞?Æ‰??êÊú¨(?¨Ë??ÜÂ??ëÈ?+?ÜÊî§Ë≤ªÁî®)/È©óÊî∂?∏È?
-- ============================================================
-- cost_functions.sql ???ΩÂú∞?êÊú¨?ÜÊî§ + ÁßªÂ??†Ê?Âπ≥Â??êÊú¨ + ÊØõÂà©?±Ë°®
-- ?®ÁΩ≤?ÜÂ?Ôº?.. -> void_functions.sql -> migration_cost.sql -> cost_functions.sql
--
-- Ë®≠Ë??üÂ?Ôº?--   1. ?ΩÂú∞?êÊú¨(?ãË≤ª/?úÁ?Á≠?‰æù„ÄåÈ?È°çÊ?‰æã„ÄçÂ??§Âà∞?∂Ë≤®?ÆÂ?Ë°??™Ë??ÑÈ???È´îÁ?,
--      ?ÖÂ??®È?È°çÊ?‰æ??•Êó•ÂæåË??πÈ???È´îÁ??ÜÊî§,?™È??øÊ??ÜÊî§?¨Â?)
--   2. ÁßªÂ??†Ê?Âπ≥Â??êÊú¨ÔºöÊ?Ê¨°Êî∂Ë≤®Â?ÔºåÁî®?åÈÄôÊâπ?ÑËêΩ?∞Ê??¨„ÄçÈ??∞Ë?ÁÆóÂ??ÅÁ?
--      Âπ≥Â??êÊú¨ÔºåÂ???inaj_t.inaj006
--   3. ?∫Ë≤®?∂‰?ËÆÄ?ñ„ÄåÁõÆ?çÂπ≥?áÊ??¨„ÄçË?ÁÆóÈÄôÁ??∑ÂîÆ?ÑÊ???COGS)ÔºåÂØ´??--      xmdl_t.xmdl006Ôºå‰?ÂæåÊ??©Â†±Ë°®Áõ¥?•Ë??ôÂÄãÊ?‰ΩçÔ?‰∏çÁî®?çÁ?
--   4. Â∑≤Áü•?êÂà∂ÔºöÂ?Ê∂àÊî∂Ë≤??ñÊ??∫Ë≤®(void_*)?ÆÂ??™È??ü„ÄåÊï∏?è„ÄçÔ?‰∏çÊ??ÜÁ?
--      Âπ≥Â??êÊú¨?ûÂà∞?ñÊ??çÁ??Ä??Âπ≥Â??êÊú¨?¨Ë≥™‰∏äÊòØÊªæÂ?Ë®àÁ?ÔºåÁ≤æÁ¢∫ÈÄÜÁ??èÁæ©‰∏çÂ§ßÔº?--      Ê≠§Á??¨Â?‰∏çË??ÜÔ??•Â??âÈ?Ë¶ÅÂ?Ë£?
-- ============================================================

-- ------------------------------------------------------------
-- Ë¶ÜÂØ´ÔºöÁ¢∫Ë™çÊî∂Ë≤®Ô??†ÂÖ•?ΩÂú∞?êÊú¨?ÜÊî§ + ÁßªÂ??†Ê?Âπ≥Â??êÊú¨Ôº?-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION confirm_goods_receipt(p_ent integer, p_docno varchar, p_user varchar)
RETURNS jsonb
LANGUAGE plpgsql
AS $$
DECLARE
  ...
  v_ap_total  numeric := 0;
  v_ap_seq    integer := 0;
  v_line_total_amt numeric := 0;   -- ?ôÊâπ?∂Ë≤®?ÑÂ??ÅÁ∏Ω?ëÈ?(?ÜÊî§?∫Á?)
  v_allocated numeric;             -- ?¨Ë??ÜÊî§?∞Á?È°çÂ?Ë≤ªÁî®
  v_landed_unit_cost numeric;      -- ?¨Ë??ΩÂú∞?Æ‰??êÊú¨
  v_old_qty   numeric;
  v_old_avg   numeric;
  v_new_avg   numeric;
BEGIN
  ...
  -- ?ôÊâπ?∂Ë≤®?ÑÂ??ÅÁ∏Ω?ëÈ?Ôºå‰??∫Â??§È?Â§ñË≤ª???ãË≤ª/?úÁ?)?ÑÊ?‰æãÂü∫Á§?  SELECT COALESCE(SUM(pmdt005), 0) INTO v_line_total_amt
  FROM pmdt_t WHERE pmdtent = p_ent AND pmdtdocno = p_docno;

  FOR v_line IN SELECT * FROM pmdt_t WHERE pmdtent = p_ent AND pmdtdocno = p_docno ORDER BY pmdtseq
  LOOP
    -- 1) ‰æùÈ?È°çÊ?‰æãÂ??§È?Â§ñË≤ª?®Ô?ÁÆóÂá∫?¨Ë??ΩÂú∞?Æ‰??êÊú¨
    ...
    -- 3) ?çÊñ∞Ë®àÁ?ÁßªÂ??†Ê?Âπ≥Â??êÊú¨
    --    fn_stock_in Â∑≤Á??äÊï∏?èÂ???inaj003ÔºåÊ?‰ª•„ÄåÂ?‰πãÂ??ÑÂ∫´Â≠òÈ??? ?ÆÂ???- ?¨Ê¨°?∂Ë≤®??    SELECT inaj003 - v_line.pmdt003, inaj006 INTO v_old_qty, v_old_avg
...
-- ------------------------------------------------------------
-- Ë¶ÜÂØ´ÔºöÁ¢∫Ë™çÂá∫Ë≤®Ô??†ÂÖ•?∫Ë≤®?∂‰??ÑÊ??¨Ë?ÁÆ?COGS)
-- ------------------------------------------------------------
CREATE OR REPLACE FUNCTION confirm_shipment(p_ent integer, p_docno varchar, p_user varchar)
RETURNS jsonb
LANGUAGE plpgsql
...
BEGIN
  ...
  FOR v_line IN SELECT * FROM xmdl_t WHERE xmdlent = p_ent AND xmdldocno = p_docno ORDER BY xmdlseq
  LOOP
    -- 1) ?∫Ë≤®?çÂ?ËÆÄ?ñ„ÄåÁõÆ?çÁ?ÁßªÂ??†Ê?Âπ≥Â??êÊú¨?çÔ?ÁÆóÂá∫?ôÁ??∑ÂîÆ?ÑÊ???COGS)
...
-- ------------------------------------------------------------
-- ÊØõÂà©?±Ë°® ViewÔºöÈÄêÁ??∫Ë≤®?éÁ¥∞?ÑÁ????êÊú¨/ÊØõÂà©/ÊØõÂà©??-- ------------------------------------------------------------
...
-- ============================================================
-- migration_user_email.sql ??ËÆ?ooag_t ?ΩÈ°ØÁ§∫Áôª?•Áî®??email
-- ?®ÁΩ≤?ÜÂ?Ôº?.. -> cost_functions.sql -> migration_user_email.sql
--
-- ?åÊôØÔºöauth.users(Supabase Auth ?ßÂª∫Ë°?Â≠òÊîæ email/ÂØÜÁ¢ºÔºåÂ?Á´ØÁÑ°Ê≥ïÁõ¥?•Êü•Ë©¢„Ä?-- Ëß??Ôºöooag_t Ë£ú‰???email Ê¨Ñ‰?ÔºåÁî®Ëß∏Áôº?®Âú®‰ΩøÁî®?ÖË®ª???ªÂÖ•?ÇËá™?ïÂ?Ê≠•È?‰æÜ„Ä?-- ============================================================

-- 1) ooag_t Ë£?email Ê¨Ñ‰?
...
-- 2) Ëß∏Áôº?®ÂáΩÂºèÔ?auth.users ?∞Â?‰ΩøÁî®?ÖÊ?ÔºåËá™?ïÂú® ooag_t Âª∫Á?/?¥Êñ∞Â∞çÊ?Ë≥áÊ?
--    SECURITY DEFINERÔºö‰ª•Âª∫Á?Ê≠§ÂáΩÂºèËÄ??öÂ∏∏?ØË??ôÂ∫´ÁÆ°Á????ÑÊ??êÂü∑Ë°åÔ?
--    ?ôÊ®£?çËÉΩÁπûÈ? ooag_t ??RLS ?êÂà∂ÂØ´ÂÖ•Ë≥áÊ?(‰∏Ä?¨‰Ωø?®ËÄÖ‰??ΩËá™Â∑±ÂØ´??ooag_t)
...
-- 3) ?õ‰?Ëß∏Áôº?®Ô?ÊØèÊ¨° auth.users ?∞Â?‰∏ÄÁ≠ÜÔ?Â∞±Ëá™?ïËß∏?º‰??¢Á??ΩÂ?
DROP TRIGGER IF EXISTS on_auth_user_created ON auth.users;
CREATE TRIGGER on_auth_user_created
  AFTER INSERT ON auth.users
  FOR EACH ROW EXECUTE FUNCTION handle_new_auth_user();

-- 4) ‰ΩøÁî®?ÖÁôª?•Ê??åÊ≠•?¥Êñ∞?ÄÂæåÁôª?•Ê??ìË?(?•Ê?ËÆäÊõ¥?? email
...
-- ============================================================
-- migration_rls_helpers.sql ??ÁÆ°Á??°Ê??êÂà§?∑Ë??©ÂáΩÂº?+ Â∏≥Ë?ÁÆ°Á??∏È? RLS
-- ?®ÁΩ≤?ÜÂ?Ôº?.. -> migration_user_email.sql -> migration_rls_helpers.sql
-- ============================================================

-- ?§Êñ∑?ÆÂ??ªÂÖ•?ÖÊòØ?¶ÁÇ∫ÁÆ°Á???‰æ?ooag_t.ooag003 Â∞çÊ? rola_t.rola002)
-- ?ôÊîØ?ΩÂ??ÉË¢´Â§öÂÄ?RLS ?øÁ??çË?‰ΩøÁî®ÔºåÁµ±‰∏ÄÂØ´‰?Ê¨°Ô?‰πãÂ??∂‰?Ë°®Ë??ãÊîæ
-- ?åÁÆ°?ÜÂì°?Ø‰ª•???πÂÖ®?®Ë??ô„ÄçÊ?ÔºåÈÉΩ?Ø‰ª•?¥Êé•?ºÂè´?ôÊîØ?ΩÂ?Ôºå‰??®È?Ë§áÂØ´?§Êñ∑?èËºØ
...
-- ?ÅË®±ÁÆ°Á??°Êõ¥?∞‰ªª‰Ωï‰∫∫??ooag_t Ë≥áÊ?(?∫Êú¨Ë≥áÊ?Á∑®ËºØ?®ÈÄ?
-- Ê≥®Ê?Ôºö‰??¨‰Ωø?®ËÄÖÂ??¨Â∞±?â„ÄåÂè™?ΩÊîπ?™Â∑±????çÁ??øÁ?(?àÂ?Â∑≤Ë®≠ÂÆ?Ôº?-- ?ôË£°?Ø„ÄåÊñ∞Â¢û„Äç‰?Ê¢ùÁµ¶ÁÆ°Á??°Á?‰æãÂ?Ë¶èÂ?ÔºåÂÖ©Ê¢ùÊîøÁ≠ñ‰∏¶Â≠òÔ?Á¨¶Â??∂‰??≥ÂèØ?öÈ?
...
-- ?ÅË®±ÁÆ°Á??°Êü•Ë©¢Ê??â‰Ωø?®ËÄ??üÊú¨?ÑÊîøÁ≠ñÂèØ?ΩÂè™?ãÊîæ?ãËá™Â∑??ôË£°?†È?ÁÆ°Á??°‰?Â§?
DROP POLICY IF EXISTS ooag_admin_select ON ooag_t;
CREATE POLICY ooag_admin_select ON ooag_t
  FOR SELECT
  USING (is_admin());
-- ============================================================
-- migration_profit_view_v2.sql ??ÊØõÂà©?±Ë°®Ë£ú‰??ÜÂ??çÁ®±/Ë¶èÊ†º
-- ?®ÁΩ≤?ÜÂ?Ôº?.. -> migration_rls_helpers.sql -> migration_profit_view_v2.sql
--
-- ?üÂ?Ôºöprofit_report_v ?üÊú¨?™Â∏∂ xmdl001(?ÜÂ?Á∑®Ë?)ÔºåÂ?Á´Ø„ÄåÈä∑?ÆÊ??©Ê?Á¥∞Ê??Æ„Ä?-- ?ÑÂ??çË??ºÊ?‰ΩçÈ?Ë¶ÅÈ°ØÁ§?imaa_t.imaa001(?ÜÂ??çÁ®±)ÔºåÊ?Ë£ú‰? JOIN??-- ============================================================
...
SELECT
  ...
  l.xmdlseq        AS line_seq,
  l.xmdl001        AS item_code,
  i.imaa001        AS item_name,   -- ?ÜÂ??çÁ®±(Â∞çÊ??®Ë??Ñ„ÄåÂ??çË??º„Ä?
  i.imaa002        AS item_spec,   -- Ë¶èÊ†º
  l.xmdl002        AS qty,
  l.xmdl003        AS unit_price,
  l.xmdl004        AS revenue,
  COALESCE(l.xmdl006, 0) AS cogs,
